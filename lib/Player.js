...
const {
  ...
  Floor,
  Wall,
} = require("./Structures/Buildings");
const { Collection } = require("discord.js");
const { Crafter, Inventory, Timeout } = require("./");
class Player extends Entity {
  ...
  constructor(id, usr, socket, game, token) {
    ...
    let getGoodPosition = (...a) => {
      let args = [...a];
      if (!args.length)
        args = ["circle", "tempx", "tempy", 150, { isStatic: true }];
      let type = args.shift();
  ...
  update() {
    ...
    if (this.chesting) {
      ...
      if (
        Vector.getDistance(this.chest.body.position, this.body.position) > 150
      )
        this.chesting = false;
    ...
    if (
      ...
      (this.craftingctable.health <= 0 ||
        Vector.getDistance(
          this.craftingctable.body.position,
          this.body.position
        ) > 150)
    )
      this.crafting = false;
    ...
    if (this.inventory.get(this.mainHand) == undefined) {
      ...
      if (
        ...
        if (/Axe/.test(this.mainHands)) {
          if (this[this.tool].timeout)
            clearTimeout(this[this.tool].timeout.timeout);
          let axerad = (this.rad / 25) * 15;
          let axep = Vector.create(0, 0);
          axep.x =
            (Math.cos((this.move.ang * Math.PI) / 180) * 70 * this.rad) / 25;
          axep.y =
            (Math.sin((this.move.ang * Math.PI) / 180) * 70 * this.rad) / 25;
          ...
          new Timeout(() => {
            rtargs.forEach((r) => {
              ...
              if (rem) {
                ...
                offset.x =
                  Math.cos((ang * Math.PI) / 180) * Vector.magnitude(offset);
                offset.y =
                  Math.sin((ang * Math.PI) / 180) * Vector.magnitude(offset);
            ...
            rtargs.forEach((r) => {
              ...
              if (
                ...
                if (
                  ...
                  this.score +=
                    this.pickaxe[u].mines[resnum].count * resnum + 2;
                }
                this.needsSelfUpdate = true;
              }
              if (rem) {
      ...
      if (
        /Wall|Campfire|Floor|Door|Crafting Table|Chest/.test(this.mainHands)
      ) {
        let mvect;
        ...
        if (/Wall|Floor|Door|Crafting Table/.test(this.mainHands)) {
                ...
                (SAT.collides(testBody, e.body).collided &&
                  !(type == "Floor"
                    ? this.structures.find(
                        (s) =>
                          e == s && type == "Floor" && !(e instanceof Floor)
                      ) ||
                      (this.clan &&
                        this.clan.members.find((member) =>
                    ...
                    : this.structures.some(
                        (s) => e == s && e instanceof Floor
                      ) ||
                      (this.clan &&
                        this.clan.members.some((member) =>
                          member.structures.some(
                            (s) => e == s && s instanceof Floor
                          )
                        )))) ||
                mvect.x < 50 ||
                mvect.y < 50 ||
          ...
          if (this.canPlace && this.move.att && !this.alusd) {
            ...
            if (/Wall|Floor|Door/.test(this.mainHands)) {
              ...
              this.structures.push(
                new CraftingTable(game, mvect.x, mvect.y, this.game)
              );
            }
            this.alusd = true;
          }
          ...
          if (this.canPlace && this.move.att && !this.alusd) {
            ...
            this.needsSelfUpdate = true;
            this.structures.push(
              new Chest(mvect.x, mvect.y, this.pang, this.game)
            );
            this.alusd = true;
          }
    ...
    if (this.hands.ready) {
      ...
      new Timeout(() => {
        rtargs.forEach((r) => {
          ...
          if (r instanceof STree) {
            rem = this.inventory.addItemMax(
              new Slot("wood", 1, "draw", 255, false)
            );
            this.score++;
            this.needsSelfUpdate = true;
          }
