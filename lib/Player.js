...
class Player extends Entity {
    ...
    constructor(id, usr, socket, game, token) {
        ...
        super(Bodies.circle(p.x, p.y, 30, {
            frictionAir: 0.02,
            restitution: 0.15,
        }), 20, game);
        //User data
    ...
    update() {
        ...
        if (this.inventory.get(this.mainHand) == undefined) {
            ...
            if (toolReg.test(this.mainHands) && this.move.att && this[this.mainHands.split(' ')[1].toLowerCase()].ready) {
                let u
    ...
    hit() {
        if (this.hands.ready) {
            ...
            let handBody = Bodies.circle(this.hposfr.x, this.hposfr.y, this.hrad, { isStatic:true })
            const { Entities } = this.game
            Entities.forEach(e => {
