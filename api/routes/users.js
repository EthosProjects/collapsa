    ...
    usersRouter
        ...
        .post(async (req, res, next) => {
            ...
            if (!username || !password || !email)
                return res.status(400).send({ message: 'invalidFormBody', error: true });
            if (collapsauserbase.findDocument(d => d.data.username == username))
                return res.status(400).send(JSON.stringify({ message: 'usernameTaken', err: true }));
            if (collapsauserbase.findDocument(d => d.data.email == email))
                return res.status(400).send(JSON.stringify({ message: 'emailTaken', err: true }));
            let valid = [0, 0, 0];
            ...
            if (valid.every(d => d)) {
                ...
                if (!valid[0] && !valid[1] && !valid[2]) res.send({ message: 'This request is faulty', err: true });
                    ...
                    res.send({ message: 'The username and password are invalid', err: true });
        ...
        .put(async (req, res) => {
            ...
            if (!collapsauserbase.findDocument(d => d.data.token == token))
                return res.send(JSON.stringify({ message: 'invalidUsername', err: true }));
            ...
            if (newPassword) {
                if (!newPassword.match(/^(?=.*[a-zA-Z0-9]).{8,16}$/))
                    return res.status(400).send(JSON.stringify({ message: 'invalidNewPassword', err: true }));
            }
            if (username != document.data.username) {
                if (!username.match(/^[a-zA-Z]{1}[ \w]{5,11}$/))
                    return res.status(400).send(JSON.stringify({ message: 'invalidUsername', err: true }));
            }
            if (email != document.data.email) {
                if (!email.match(/^\w.+@\w{2,253}\.\w{2,63}$/))
                    return res.status(400).send(JSON.stringify({ message: 'invalidEmail', err: true }));
            }
        ...
        if (collapsauserbase.documents.has(req.params.userid)) {
            if (req.headers['authorization']) {
                ...
                if (req.headers['authorization'] == `Basic ${user.token}`) {
                    res.send(
                        JSON.stringify(
                            new collapsauserbaseUser(collapsauserbase.documents.get(req.params.userid).data).private
                        )
                    );
                ...
                res.send(
                    JSON.stringify(
                        new collapsauserbaseUser(collapsauserbase.documents.get(req.params.userid).data).public
                    )
                );
