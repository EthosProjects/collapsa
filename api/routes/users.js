...
module.exports = (mongoDB) => {
  ...
  usersRouter
    .route("/")
    .get(async (req, res) => {
      const collapsauserbase = mongoDB.databases
        .get("collapsa")
      ...
      if (req.query.discord) {
        const discorduserbase = mongoDB.databases
          .get("collapsa")
          ...
          discorduserbase.filterDocuments((document) => {
            ...
            if (
              !collapsauserbase.findDocument(
                (doc) => doc.data.discordid == document.name
              )
            )
              return false;
            return true;
    ...
    .post(async (req, res, next) => {
      ...
      if (!username || !password || !email)
        return res
          .status(400)
          .send({ message: "invalidFormBody", error: true });
      if (collapsauserbase.findDocument((d) => d.data.username == username))
        return res
          .status(400)
          .send(JSON.stringify({ message: "usernameTaken", err: true }));
      if (collapsauserbase.findDocument((d) => d.data.email == email))
        return res
          .status(400)
          .send(JSON.stringify({ message: "emailTaken", err: true }));
      let valid = [0, 0, 0];
      ...
      if (valid.every((d) => d)) {
        ...
        token =
          "Aph_" +
          (random.length == 2 ? random : "0" + random) +
          "yght" +
          token.substr(7);
        ...
        if (!valid[0] && !valid[1] && !valid[2])
          res.send({ message: "This request is faulty", err: true });
        else if (!valid[0] && !valid[1])
          res.send({
            message: "The username and password are invalid",
            err: true,
          });
        else if (!valid[0] && !valid[2])
          res.send({
            message: "The username and email are invalid",
            err: true,
          });
        else if (!valid[1] && !valid[2])
          res.send({
            message: "The password and email are invalid",
            err: true,
          });
        else if (!valid[0])
          res.send({ message: "The username is invalid", err: true });
        else if (!valid[1])
          res.send({ message: "The password is invalid", err: true });
        else if (!valid[2])
          res.send({ message: "The email is invalid", err: true });
      }
      res.end();
    })
    .put(async (req, res) => {
      ...
      if (!collapsauserbase.findDocument((d) => d.data.token == token))
        return res.send(
          JSON.stringify({ message: "invalidUsername", err: true })
        );
      let document = collapsauserbase.findDocument(
      ...
      if (newPassword) {
        if (!newPassword.match(/^(?=.*[a-zA-Z0-9]).{8,16}$/))
          return res
            .status(400)
            .send(JSON.stringify({ message: "invalidNewPassword", err: true }));
      }
      if (username != document.data.username) {
        if (!username.match(/^[a-zA-Z]{1}[ \w]{5,11}$/))
          return res
            .status(400)
            .send(JSON.stringify({ message: "invalidUsername", err: true }));
      }
      if (email != document.data.email) {
        if (!email.match(/^\w.+@\w{2,253}\.\w{2,63}$/))
          return res
            .status(400)
            .send(JSON.stringify({ message: "invalidEmail", err: true }));
      }
  ...
  usersRouter.get("/link", async (req, res, next) => {
    ...
    let redirect_uri = `https://${req.hostname}${
      process.env.NODE_ENV == "development" ? `:${process.env.PORT}` : ""
    }/api/v1/users/link?site=discord`;
    console.log(req);
    ...
    if (getCookie("token")) {
      if (
        collapsauserbase.findDocument(
          (document) => document.data.token == getCookie("token")
        )
      ) {
      ...
      collapsauserbase.findDocument(
        (document) => document.data.discordid == user.id
      )
    ) {
      let cUser = new collapsauserbaseUser(
        collapsauserbase.findDocument(
      ...
      if (
        collapsauserbase.findDocument((d) => d.data.username == user.username)
      )
        return res
          .status(400)
          .send(JSON.stringify({ message: "usernameTaken", err: true }));
  ...
  usersRouter.get("/:userid", (req, res, next) => {
    ...
    if (collapsauserbase.documents.has(req.params.userid)) {
      if (req.headers["authorization"]) {
        let user = new collapsauserbaseUser(
          collapsauserbase.documents.get(req.params.userid).data
        );
        if (req.headers["authorization"] == `Basic ${user.token}`) {
          res.send(
            JSON.stringify(
              new collapsauserbaseUser(
                collapsauserbase.documents.get(req.params.userid).data
              ).private
            )
          );
  ...
  usersRouter.post("/login", async (req, res) => {
    ...
    if (!collapsauserbase.findDocument((d) => d.data.username == username))
      return res.send(
        JSON.stringify({ message: "Incorrect username or password", err: true })
      );
    let document = collapsauserbase.findDocument(
      (d) => d.data.username == username
    );
    let user = new collapsauserbaseUser(document.data);
    if (!document.data.password) {
      let acceptedLogins = [];
    ...
    if (successful) {
      ...
      res.send(JSON.stringify({ message: "Incorrect username or password" }));
  });
  return usersRouter;
};
