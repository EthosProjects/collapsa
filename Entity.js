...
const {
  ...
  Stone,
  STree,
} = require("./lib/Structures/Resources");
const {
  Campfire,
  Chest,
  CraftingTable,
  ...
  Floor,
  Wall,
} = require("./lib/Structures/Buildings");
...
module.exports = function (nsp, ns, mLab) {
      ...
      Walls.list.forEach((wall) => {
        if (wall.health <= 0) {
          ...
          if (Players.list.find((p) => p.structures.find((s) => s == wall))) {
            let p = Players.list.find((p) =>
              p.structures.find((s) => s == wall)
            );
            p.structures.splice(
    ...
    update: function () {
      ...
      Campfires.list.forEach((campfire) => {
        if (campfire.health <= 0) {
          ...
          if (
            Players.list.find((p) => p.structures.find((s) => s == campfire))
          ) {
            let p = Players.list.find((p) =>
              p.structures.find((s) => s == campfire)
            );
            p.structures.splice(
    ...
    update: () => {
      ...
      for (var i = 0; i < Demons.list.length; i++) {
        ...
        if (timeOfDay == "day") {
          ...
          demon._maxSpd =
            (-2 * Math.abs(dayTimeout.percntDone - 0.5) + 1) * 1 + 1;
        } else {
          demon.hands.damage =
    ...
    update: () => {
      ...
      for (var i = 0; i < Destroyers.list.length; i++) {
        ...
        if (timeOfDay == "day") {
          ...
          demon._maxSpd =
            (-2 * Math.abs(dayTimeout.percntDone - 0.5) + 1) * 0.5 + 1;
        } else {
          demon.hands.damage =
            4 + (-2 * Math.abs(dayTimeout.percntDone - 0.5) + 1) * 2;
          demon._maxSpd =
            1.75 + (-2 * Math.abs(dayTimeout.percntDone - 0.5) + 1);
        }
  ...
  let getGoodPosition = (...a) => {
    let args = [...a];
    if (!args.length)
      args = ["circle", "tempx", "tempy", 50, { isStatic: true }];
    let type = args.shift();
  ...
  this.nsp.on("connection", function (socket) {
    ...
    socket.on("startInitPackReq", () => {
      ...
      Diamonds.list.forEach((diamond) =>
        pack.diamond.push(diamond.getInitPack())
      );
