const fs = require('fs');
const { MessageEmbed, Webhook, WebhookClient, TextChannel, Collection } = require('discord.js');
//
...
module.exports = mLab => {
    ...
    mLab.once('ready', () => {
        let discorduserbase = mLab.databases.get('collapsa').collections.get('discorduserbase');
    ...
    client.on('message', async message => {
        ...
        if (message.content.startsWith('!joinGuild') && message.author.id == client.owner) {
        ...
        if (antiSpam.has(author.id + message.guild.id)) {
            ...
            if (usr.count >= 5) {
                if (channel.permissionsFor(message.guild.me).has('DELETE_MESSAGES')) message.delete();
                message.channel.send("You're sending messages too fast!");
            }
            usr.count++;
            setTimeout(() => {
                antiSpam.get(author.id + message.guild.id).count--;
                if (!antiSpam.get(author.id + message.guild.id).count) antiSpam.delete(author.id + message.guild.id);
            }, 2000);
        } else {
        ...
        if (client.commands.has(commandName)) {
            ...
            if (!member.hasPermission(command.permissions) && member.id != client.owner)
                return message.reply(`You don't have permission to use this command`);
            if (!guild.me.hasPermission(command.permissions) && command.permissions != ['ADMINISTRATOR'])
    ...
    client.on('guildCreate', async guild => {
        ...
        defaultChannel.send("Hello! It's CollapsaBot. Use !configure to begin configuration");
    });
    client.login(
        process.env.NODE_ENV == 'production' ? token : 'NzE3OTU5MzYyMTMxMjYzNjA5.Xty2Pg.exCv_gp7VQbgPWK11TtTBGereJ8'
    );
