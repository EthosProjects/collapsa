  ...
  async search(term, max, order) {
    return new Promise(async (resolve) => {
      https
        .request({
          host: "www.googleapis.com",
          path: `/youtube/v3/search?key=${
            this.apiKey
          }&part=snippet&q=${term}&maxResults=${max || 10}&order=${order}`,
          method: "GET",
          headers: {
            "content-type": "application/json",
          },
      ...
      resolve(
        base.items.map((item) => {
          ...
          if (item.id.kind == "youtube#playlist")
            return new Playlist(item, this.apiKey);
          if (item.id.kind == "youtube#video")
            return new Video(item, this.apiKey);
          if (item.id.kind == "youtube#channel")
            return new Channel(item, this.apiKey);
          ...
          rp({
            ...
            if (res["content-length"] > limit) reject("File too big");
              ...
              video.on("complete", function complete(info) {
                "use strict";
                console.log(
                  "filename: " + info._filename + " already downloaded."
                );
                resolve();
              });
