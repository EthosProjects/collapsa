...
module.exports = {
    ...
    execute: async (message, args = [], client, mLab) => {
        ...
        if(!guildbase) return message.reply('This guild is not in the database yet. Please wait')
        ...
        switch(args[0]){
            case 'mutedRole' :
                ...
                message.channel.awaitMessages(validRoleFilter, { max:1, time: 15000 })
                    .then(async mc => {
                        let m = mc.first()
                        if(m.content == 'no'){
                            await message.channel.send('No muted role: true\nConfiguration complete. Please wait')
                        }else {
                            let role = message.guild.roles.cache.get(m.content) || message.guild.roles.cache.get(m.mentions.roles.first().id)
            ...
            case 'moderationChannel' :
                await message.channel.send('Please select a channel to send moderation logs')
                message.channel.awaitMessages(validChannelFilter, { max:1, time: 15000 })
                    .then(async mc => {
                        let m = mc.first()
                        if(m.content == 'no'){
                            await message.channel.send('No moderation channel selected\nConfiguration complete. Please wait')
                        }else {
                            let channel = message.guild.channels.cache.get(m.content) || message.guild.channels.cache.get(m.mentions.channels.first().id)
                            doc.moderation.channel = channel.id
                            await message.channel.send(`Set moderation channel to ${channel.name}\nConfiguration complete. Please wait`)
                        }
                    })
                    .catch(async () => {
                        await message.channel.send('No moderation channel\nConfiguration complete. Please wait')
                    })
                    .finally(async () => {
                        await discordguildbase.updateDocument(doc)
                            ...
                            .finally(() => {
                                message.channel.awaitMessages(validChannelFilter, { max:1, time: 15000 })
                                    .then(async mc => {
                                        let m = mc.first()
                                        if(m.content == 'no'){
                                            await message.channel.send('No welcome channel selected\nMoving to next item\nPlease select an autorole')
                                        }else {
                                            let channel = message.guild.channels.cache.get(m.content) || message.guild.channels.cache.get(m.mentions.channels.first().id)
                                            doc.welcome.channel = channel.id
                                            await message.channel.send(`Set welcome channel to ${channel.name}\nMoving to next item\nPlease select an autorole`)
                                        }
                                    })
                                    .catch(async () => {
                                        await message.channel.send('No welcome channel selected\nMoving to next item\nPlease select an autorole')
                                    ...
                                    .finally(() => {
                                        message.channel.awaitMessages(validRoleFilter, { max:1, time: 15000 })
                                            .then(async mc => {
                                                let m = mc.first()
                                                if(m.content == 'no'){
                                                    await message.channel.send('No autorole selected\nConfiguration complete. Please wait')
                                                }else {
                                                    let role = message.guild.roles.cache.get(m.content) || message.guild.roles.cache.get(m.mentions.roles.first().id)
                                                    doc.welcome.role = role.id
                                                    await message.channel.send(`Set autorole to ${role.name}\nConfiguration complete. Please wait`)
                                                }
                                            })
                                            .catch(async () => {
                                                await message.channel.send('No autorole selected\nConfiguration complete. Please wait')
                                            ...
                                            .finally(async () => {
                                                await discordguildbase.updateDocument(doc)
                                                await message.channel.send(`You may now use your commands of choice`)
                                            })
                                    })
                            })
                    })
