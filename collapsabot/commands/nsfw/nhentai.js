    ...
    switch (type) {
      case "search":
        ...
        if (first) {
          ...
          let badReactionFilter = (reaction, user) =>
            ...
            if (reaction.emoji.name == "➡️") {
              ...
              embed.setTitle(
                (book.title.pretty ||
                  ...
                  (pageNum == 1
                    ? ": Cover Page"
                    : `Page ${pageNum} out of ${book.pages.length}`)
              );
          ...
          let books = results.books
            .filter((book) =>
              book.tags.some(
                (tag) => tag.type.type == "language" && tag.name == "english"
              )
            )
            .splice(0, 10);
          ...
          if (embed.length > 6000)
            return message.channel.send(
              "Unfortunately, this query is unnacceptable. Please try again"
            );
          let m = await message.channel.send(embed);
        }
        break;
