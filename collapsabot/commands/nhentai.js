        ...
        switch(type){
            case 'search': 
                ...
                if(first){
                    ...
                    m.react('➡️')
                    m.react('❌')
                    let filter = (reaction, user) => (reaction.emoji.name == '⬅️' || reaction.emoji.name == '➡️' || reaction.emoji.name == '❌') && user.id == message.author.id
                    ...
                    collector.on('collect', reaction => {
                        ...
                        if(reaction.emoji.name == '➡️'){
                            ...
                            pageNum++
                            embed.setTitle((book.title.pretty || book.title.english || book.title.japanese) + (pageNum == 1 ? ': Cover Page' : `Page ${pageNum} out of ${book.pages.length}`))
                            ...
                            embed.setImage('')
                            embed.setTitle(`Thank you for reading ${book.title.pretty || book.title.english || book.title.japanese}`)
                            m.edit(embed)
                    ...
                    let books = results.books.filter(book => book.tags.some(tag => tag.type.type == 'language' && tag.name == 'english')).splice(0, 10)
                    let embed = new MessageEmbed()
                    ...
                    books.forEach(book => {
                        ...
                        embed
                            ...
                            .addField('Tags', tags.length > 10 ? tags.splice(0, 10).map(tag => '`' + tag.name + '`').join(' ') + '...' : tags.map(tag => tag.name).join(' '), true)
                    })
                    if(embed.length > 6000) return message.channel.send('Unfortunately, this query is unnacceptable. Please try again')
                    let m = await (
