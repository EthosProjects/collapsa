  ...
  execute: async (message, args = [], client, mLab) => {
    ...
    if (!command || (!message.channel.nsfw && command.nsfw)) {
      ...
      client.commandFolders.forEach((commands, name) => {
        ...
        let commandList = commands.reduce(
          (a, b) => `${a} ${b.name}`,
          firstCommand.name
        );
      ...
      message.channel.send(
        "Use `!help commandname` to get information on a specific command"
      );
      return;
    } else if (!argument) {
      ...
      embed.addField(
        "Roles",
        roles.startsWith("@everyone") ? "@everyone" : roles
      );
      message.channel.send(embed);
      message.channel.send(
        "Use `" +
          `!help ${command.name} argumentname` +
          "` to get information on a specific argument"
      );
    } else {
