const { MessageEmbed, Message, Collection, Client, GuildMember } = require('discord.js');
...
module.exports = new Command({
    ...
    execute: async (message, args = [], client, mLab) => {
        ...
        let guildSetup = mLab.databases.get('collapsa').collections.get('discordguildbase').documents.get(guild.id)
            .data;
        ...
        if (!discorduserbase.documents.has(member.id)) {
            ...
            return message.channel.send('It is physically impossible for this bot to have muted that user');
        ...
        let botRolesArr = [...guild.members.cache.get(client.user.id).roles.cache.values()]
        ...
        if (
            ...
            return message.reply("The bot doesn't have permission to assign this role");
        ...
        if (author.id == client.owner || author.id == guild.ownerID) {
            ...
            let embed = new MessageEmbed()
                ...
                .addFields([{ name: 'Reason', value: reason ? reason : 'No reason specified', inline: true }])
        ...
        if (authorRolesArr[0] <= memberRolesArr[0])
            return message.reply("You can't mute this user your role is not high enough");
