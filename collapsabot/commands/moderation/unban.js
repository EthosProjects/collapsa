...
module.exports = new Command({
  ...
  arguments: [
    new Argument({
      ...
      type: "UserID",
      description:
        "User ID, mention, or username of the user whom you want unban",
    }),
    new Argument({
      _name: "reason",
      nameStartsWithVowel: false,
  ...
  execute: async (message, args = [], client, mLab) => {
      ...
      guild.members.unban(user).then(
        (res) => {
          let embed = new MessageEmbed()
            ...
            .setThumbnail(
              "http://www.collapsa.io/client/img/amethysthammer.png"
            )
    ...
    if (!guild.owner.id == user)
      return message.reply(
        "You can't ban this user because they own the server"
      );
    guild.members.unban(user).then(
      (res) => {
        let embed = new MessageEmbed()
