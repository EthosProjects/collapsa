    ...
    execute: async (message, args = [], client, mLab) => {
        ...
        if(!moderation){
            let embed = new MessageEmbed()
                .setColor('#000001')
                .setTitle(`Moderation ${member.user.username} has currently been moderated ${user.guilds[guild.id].moderation.filter(m => !m.removed).length} time${user.guilds[guild.id].moderation.filter(m => !m.removed).length == 1 ? '' : 's'}`)
                .setAuthor(message.author.username, message.author.avatarURL())
            user.guilds[guild.id].moderation.filter(m => !m.removed).forEach(moderation => {
                embed.addFields([
                    { name: 'Type', value: moderation.type, inline:true},
                    { name: 'Reason', value: moderation.reason, inline:true},
            ...
            if(moderationCommands.find(c => c == args[2])){
                if(args[2] == 'remove'){
                    ...
                    let embed = new MessageEmbed()
                        .setColor('#000001')
                        .setTitle(`Removed moderation ${moderation.id}`)
                    message.channel.send(embed)
                }else if(args[3]){
