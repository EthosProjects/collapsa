...
module.exports = new Command({
    ...
    execute: async (message, args = [], client, mLab) => {
        ...
        if (!member.kickable)
            return message.reply("You can't kick this user because the bot does not have sufficient permissions!");
        if (!guild.owner.id == member.id.id)
            return message.reply("You can't kick this user because they own the server");
        ...
        if (authorRolesArr[0] <= memberRolesArr[0])
            return message.reply("You can't kick this user your role is not high enough");
