...
module.exports = new Command({
  ...
  arguments: [
    new Argument({
      ...
      type: "Member",
      description:
        "User ID, mention, or username of the Member whom you want warn",
    }),
    new Argument({
      _name: "reason",
      nameStartsWithVowel: false,
  ...
  execute: async (message, args = [], client, mLab) => {
    ...
    if (author.id == client.owner || author.id == guild.ownerID) {
      ...
      let embed = new MessageEmbed()
        ...
        .addFields([
          ...
          {
            name: "Current warnings count",
            value: user.guilds[guild.id].moderation.filter(
    ...
    if (authorRolesArr[0] <= memberRolesArr[0])
      return message.reply(
        "You can't warn this user your role is not high enough"
      );
    if (!guild.owner.id == member.id)
      return message.reply(
        "You can't warn this user because they own the server"
      );
    let user = new discorduserbaseUser(
      discorduserbase.documents.get(member.id).data
    );
    ...
    let embed = new MessageEmbed()
      ...
      .addFields([
        ...
        {
          name: "Current warnings count",
          value: user.guilds[guild.id].moderation
            .filter((m) => !m.removed)
            .filter((warning) => warning.type == "Warning").length,
          inline: true,
        },
      ])
      .setAuthor(message.author.username, message.author.avatarURL());
