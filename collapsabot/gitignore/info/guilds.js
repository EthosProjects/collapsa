...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        let msg = client.guilds.cache.map((guild) => `**${guild.name}** Members: ${guild.memberCount}`).join('\n');
        if (!target) return message.reply('Only the bot Owner <@436016533454454784> Can use this Command. sorry');
        else {
