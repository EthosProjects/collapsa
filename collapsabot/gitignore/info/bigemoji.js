...
module.exports = {
	...
	run: async (client, message, args = []) => {
		...
		if (!args[0].match(emojiRegex))
			return message.reply('No emoji specified?');
		let id = args[0].replace(/<a{0,1}:[a-zA-Z0-9_]{2,32}:/, '').replace(/>/, '');
    	let embed = new Discord.MessageEmbed()
      	.setTitle('Here\'s the Emoji')
			.setColor(randomColor)
			.setImage(`https://cdn.discordapp.com/emojis/${id}.${args[0].match(/<a:/) ? 'gif' : 'png'}`);
		console.log(`https://cdn.discordapp.com/emojis/${id}.${args[0].match(/<a:/) ? 'gif' : 'png'}`)
