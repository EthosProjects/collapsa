...
module.exports = {
    ...
    run: async (client, message, args) => {
        const logChannel = message.guild.channels.cache.find((c) => c.name === 'duke-logs') || message.channel;

        if (message.deletable) message.delete();
        // No args
        if (!args[0]) {
            return message.reply('Please provide a person to Kick.').then((m) => m.delete());
        }

        // No reason
        if (!args[1]) {
            return message.reply('Please provide a reason to Kick.').then((m) => m.delete());
        }

        // No author permissions
        if (!message.member.hasPermission('KICK_MEMBERS')) {
            return message
                .reply('❌ You do not have permissions to Kick members. Please contact a staff member')
                .then((m) => m.delete());
        }
        // No bot permissions
        if (!message.guild.me.hasPermission('KICK_MEMBERS')) {
            return message
                .reply('❌ I do not have permissions to Kick members. Please contact a staff member')
                .then((m) => m.delete());
        }
        ...
        // No member found
        if (!toKick) {
            return message.reply("Couldn't find that member, try again").then((m) => m.delete());
        }
