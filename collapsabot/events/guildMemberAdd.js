...
module.exports = new Event({
    ...
    execute: (member, client) => {
        ...
        if (mLab.databases.get('collapsa').collections.get('discorduserbase').documents.has(member.id)) {
            let mData = mLab.databases.get('collapsa').collections.get('discorduserbase').documents.get(member.id);
            if (mData.data.muteTimeEnd && guildSetup.mute.role && member.guild.roles.cache.has(guildSetup.mute.role)) {
        ...
        if (guildSetup.welcome.role) {
            let role = member.guild.roles.cache.find((r) => r.id == guildSetup.welcome.role);
