      ...
      for (let i = 0; i < this.arguments.length; i++) {
        ...
        let resolveTime = (argument) => {
          ...
          let timeRegex = new RegExp(
            `^\\d+(${times.reduce((a, b) => `${a}|${b}`)})$`
          );
          let timeReplaceRegex = new RegExp(
          ...
          let initialResolve = (arg) => {
            if (args[argIndex].match(/^\d{16,18}$/)) userID = args[argIndex];
              ...
              message.guild.members.cache.find(
                (member) =>
                  member.nickname == args[argIndex] ||
                  member.user.username == args[argIndex]
              )
            ) {
        ...
        let resolveEmoji = (arg) => {
          ...
          let id = args[argIndex]
            .replace(/<:[a-zA-Z0-9_]{2,32}:/, "")
            .replace(/>/, "");
        ...
        let resolveArgumentName = (argument) => {
          ...
          if (client.commands.has(previousArg)) {
            ...
            if (command.arguments.find((argument) => argument._name == currArg))
              return command.arguments.find(
                (argument) => argument._name == currArg
              );
            else return new Invalid();
          } else return new Invalid();
        };
        ...
        let resolveArgument = (argument) => {
          if (!currArg) {
            if (argument.type == "Time")
              return (resolvedArgs[i] = 60 * 60 * 1000);
