            ...
            for(let i = 0; i < this.arguments.length;i++){
                ...
                let resolveUser = arg => {
                    ...
                    let initialResolve = arg => {
                        ...
                        }else if(message.guild.members.cache.find(member => member.nickname == args[argIndex] || member.user.username == args[argIndex])){
                            userID = message.guild.members.cache.find(member => member.nickname == args[argIndex] || member.user.username == args[argIndex]).user.id
                        }else userID = false
                        if(userID){
                ...
                let resolveArgumentName = argument => {
                    ...
                    if(client.commands.has(previousArg)){
                        ...
                        if(command.arguments.find(argument => argument._name == currArg)) return command.arguments.find(argument => argument._name == currArg)
                        else return new Invalid
                    }else return new Invalid()
                }
                /**
