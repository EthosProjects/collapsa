...
var init = function (name) {
    ...
    handlemouseDown = e => {
        ...
        if (playa.armor) {
            if (e.clientX > 10 && e.clientX < 10 + 90 && e.clientY > 350 && e.clientY < 350 + 90) {
        ...
        if (playa.hat) {
            if (e.clientX > 10 && e.clientX < 10 + 90 && e.clientY > 240 && e.clientY < 240 + 90) {
                ctx.fillStyle = '#696969';
                ctx.beginPath();
                ctx.lineWidth = 1.5;
                ctx.globalAlpha = 0.75;
        ...
        if (playa.clanning && playa.clans && !playa.clan) {
            ...
            playa.clans.forEach((name, i) => {
                ...
                if (
                    ...
                    e.clientX < 400 + (canvas.width - 820) / 4 + offSetX + (canvas.width - 820) / 4 + 2.5 &&
        ...
        if (playa.crafting && playa.craftablesEx) {
            categories.forEach((c, i) => {
                if (e.clientX > 350 && e.clientX < 450 && e.clientY > 100 + 100 * i && e.clientY < 200 + 100 * i)
                    category = c[0];
            });
            playa.craftablesEx
    ...
    */
    ...
    class Player {
        ...
        draw(x, y) {
            ...
            ctx.fill();
            if (this.armor)
                ctx.drawImage(Img[this.armor], currx - this.rad, curry - this.rad, this.rad * 2, this.rad * 2);
            ctx.translate(currx, curry);
            ctx.rotate((Math.PI / 180) * this.angle);
            ctx.fillStyle = 'black';
            ctx.beginPath();
        ...
        if (!receivedFirstUpdate || !allLoaded) {
            ...
            dropped.forEach(item => {
                ...
                if (/^Leather$/.test(item.slot.type))
                    ctx.drawImage(Img[item.slot.image], item.x + x - 24.5, item.y + y - 28.5, 49, 57);
            ...
            if (playa.chesting && playa.chestables) {
                ...
                playa.chestables.forEach((item, i) => {
                    ...
                    if (/^leather$/.test(item.type))
                        ctx.drawImage(Img[item.image], item.x + x - 24.5, item.y + y - 28.5, 49, 57);
