    ...
    for (let prop in Img) {
      if (typeof Img[prop] == "string") {
        ...
        if (prop.match(/^(wood|stone|iron)wall$/)) {
          ...
          let wallTypes = ["oneway", "twoway", "threeway", "fourway", "corner"];
          wallTypes.forEach((t) => {
...
var init = function (name) {
  ...
  handlemouseDown = (e) => {
    ...
    playa.inventory.forEach((slot, i) => {
      if (
        e.clientX > canvas.width / 10 + (canvas.width / 10) * i - 45 &&
        e.clientX < canvas.width / 10 + (canvas.width / 10) * i - 45 + 90 &&
        e.clientY > canvas.height - 100 - 45 &&
        e.clientY < canvas.height - 100 - 45 + 90
      ) {
        found = true;
        _dragging = [slot, i];
    ...
    if (playa.chesting) {
      playa.chestables.forEach((item, i) => {
        ...
        if (
          e.clientX > (canvas.width - 300) / 2 + 40 + offSetX &&
          e.clientX < (canvas.width - 300) / 2 + 40 + offSetX + 60 &&
          e.clientY > (canvas.height - 300) / 2 + 40 + offSetY &&
          e.clientY < (canvas.height - 300) / 2 + 40 + offSetY + 60
        ) {
    ...
    if (found) return;
    if (
      Math.sqrt(Math.pow(e.clientX - 290, 2) + Math.pow(e.clientY - 130, 2)) <=
      50
    ) {
    ...
    if (playa.clanning && playa.clans && !playa.clan) {
      if (
        e.clientX > 410 + (canvas.width - 820) / 2 &&
        e.clientX < 410 + (canvas.width - 820) / 2 + (canvas.width - 820) / 2 &&
        e.clientY > 110 &&
        e.clientY < 110 + 24 &&
        clanbox.value().length
      ) {
        ctx.fillRect(
          410 + (canvas.width - 820) / 2,
          110,
          (canvas.width - 820) / 2,
          24
        );
      ...
      playa.clans.forEach((name, i) => {
        ...
        if (
          e.clientX > 400 + (canvas.width - 820) / 4 + offSetX &&
          e.clientX <
            400 +
              (canvas.width - 820) / 4 +
              offSetX +
              (canvas.width - 820) / 4 +
              2.5 &&
          e.clientY > 110 + 26 + offSetY &&
          e.clientY < 110 + 26 + offSetY + 22
        ) {
          ...
          ctx.fillRect(
            400 + (canvas.width - 820) / 4 + offSetX,
            110 + 26 + offSetY,
            (canvas.width - 820) / 4 + 2.5,
            22
          );
        }
      });
    ...
    if (
      playa.clanning &&
      playa.clan &&
      e.clientX > 400 + canvas.width - 900 &&
      e.clientX < 400 + canvas.width - 900 + 75 &&
      e.clientY > canvas.height - 150 &&
      e.clientY < canvas.height - 150 + 25
    ) {
    ...
    if (playa.owner && playa.clanning) {
      playa.clanMembers.forEach((player, i) => {
        ...
        if (
          e.clientX > 500 + (canvas.width - 1000) / 2 - 75 + offSetX &&
          e.clientX < 500 + (canvas.width - 1000) / 2 - 75 + offSetX + 50 &&
          e.clientY > 200 - 13.5 + offSetY &&
          e.clientY < 200 - 13.5 + offSetY + 15
        ) {
          ...
          ctx.beginPath();
          ctx.rect(
            500 + (canvas.width - 1000) / 2 - 75 + offSetX,
            200 - 13.5 + offSetY,
            50,
            15
          );
    ...
    if (playa.crafting && playa.craftablesEx) {
      ...
      playa.craftablesEx
        .filter((craft) => TableRecipes.get(craft.craft).type == category)
        .forEach((craft, i) => {
          let offSetX = (i - Math.floor(i / 8) * 8) * 80;
          let offSetY = Math.floor(i / 8) * 80;
          if (
            e.clientX > offSetX + 470 &&
            e.clientX < offSetX + 470 + 60 &&
            e.clientY > offSetY + 120 &&
            e.clientY < offSetY + 120 + 60
          ) {
            found = true;
  ...
  handlemouseMove = function (e) {
    ...
    playa.inventory.forEach((slot, i) => {
      if (
        ...
        if (playa.inventory[i].type == "wood")
          return (hover = [Resources.get("wood"), i]);
  ...
  class miniPlayer {
    ...
    drawPerf(opt) {
      ...
      ctx.fillRect(
        currx - (40 * this.rad) / 25,
        curry - (50 * this.rad) / 25,
        hpBar,
        10
      );
      ctx.fillStyle = "blue";
      if (opt.drawStamina)
        var staminaBar =
          (((80 * this.rad) / 25) * this.stamina) / this.maxStamina;
      ctx.fillRect(
        currx - (40 * this.rad) / 25,
        curry - 2 * this.rad + 10,
        staminaBar,
      ...
      if (this.mainHand == "hand") {
        if (!this.rhit) {
          ...
          ctx.rotate(
            (Math.PI / 180) *
              (360 - (-Math.abs(-160 * this.punchper + 80) + 80))
          );
          ctx.beginPath();
          ctx.fillStyle = "black";
        ...
        if (!this.lhit) {
          ...
          ctx.translate(32 - 7.5, -(15 - 7.5));
          ctx.rotate(
            (Math.PI / 180) * (0 + (-Math.abs(-160 * this.punchper + 80) + 80))
          );
          ctx.beginPath();
          ctx.fillStyle = "black";
          ctx.arc(7.5, -7.5, 7.5, 0, 2 * Math.PI);
        ...
        if (/Axe|Pickaxe|Shovel|Sword|Hammer/.test(this.mainHand)) {
          ...
          if (/Axe/.test(this.mainHand)) {
            ...
            if (this.hitting)
              ctx.rotate(
                (Math.PI / 180) * (360 - (-Math.abs(-120 * this.per + 60) + 60))
              );
            ctx.save();
        ...
        if (
          /Wall|Campfire|Door|Floor|Crafting Table|Chest/.test(this.mainHand)
        ) {
          let img = this.mainHand.toLowerCase().replace(/\s/, "");
          drawHand(32 - 7.5, -15 - 7.5, 15, 15);
          ctx.save();
        ...
        if (this.mainHand == "carrot") {
          ...
          if (this.per)
            r =
              (Math.PI / 180) * (360 - (-Math.abs(-160 * this.per + 80) + 80));
          else r = 0;
          if (r < 180) r += 180;
          else r -= 180;
          ctx.rotate(r);
    ...
    draw(x, y) {
      ...
      if (
        ...
        if (/Wall|Campfire|Floor|Crafting Table/.test(this.mainHand)) {
          ctx.drawImage(
            Img[img],
            this.posPlace.x - 50 + x,
            this.posPlace.y - 50 + y,
            100,
            100
          );
          if (!playa.canPlace) {
  ...
  class Demon {
    ...
    draw(x, y) {
      ...
      if (!this.rhit) {
        ...
        ctx.translate(32 - 7.5, 15 - 7.5);
        ctx.rotate(
          (Math.PI / 180) * (360 - (-Math.abs(-160 * this.punchper + 80) + 80))
        );
        ctx.beginPath();
        ctx.fillStyle = "black";
        ctx.arc(7.5, 7.5, 7.5, 0, 2 * Math.PI);
  ...
  class Stone {
    ...
    show(x, y) {
      drawStone(this.x + x, this.y + y, 90);
        ...
        if (
          ...
          if (
            ...
            if (Walls.find((w) => w.x == this.x && w.y == this.y + 100)) {
              ctx.drawImage(
                Img[this.material + "wallthreeway"],
                -50,
                -50,
                100,
                100
  ...
  class Door {
    ...
    show(x, y) {
      if (this.ang == "up") {
        ...
        if (this.per && !this.open)
          ctx.rotate((180 * this.per * Math.PI) / 180);
        if (this.per && this.open)
          ctx.rotate(((180 - 180 * this.per) * Math.PI) / 180);
  ...
  class Chest {
    ...
    show(x, y) {
      ...
      ctx.fillStyle = "red";
      ctx.translate(this.x + x, this.y + y);
      if (this.ang == "left" || this.ang == "right")
        ctx.rotate((Math.PI / 180) * 90);
  ...
  let drawAxis = () => {
    ...
    yGradient.addColorStop(0.5, "purple");
  ...
  readRemovePack = function (pack) {
    pack.player.forEach(function (id) {
      Players.splice(
        Players.findIndex(function (element) {
    ...
    pack.demon.forEach(function (id) {
      Demons.splice(
        Demons.findIndex(function (element) {
          return element.id == id;
        }),
        1
      );
    ...
    pack.destroyer.forEach(function (id) {
      Destroyers.splice(
        Destroyers.findIndex(function (element) {
          return element.id == id;
        }),
        1
      );
    ...
    pack.rabbit.forEach(function (id) {
      Rabbits.splice(
        Rabbits.findIndex(function (element) {
          return element.id == id;
        }),
        1
      );
  ...
  let initPacks = [];
  let removePacks = [];
  socket.on("initPack", (pack) => {
    initPacks.push(pack);
  });
  socket.on("removePack", (pack) => {
    removePacks.push(pack);
  });
  ...
  readPack = (pack) => {
    ...
    if (!receivedFirstUpdate || !allLoaded) {
      ...
      if (!loadingTimerRefresh)
        loadingTimerRefresh = setTimeout(() => {
          //location.reload()
        }, 5000);
    } else if (playa) {
      ...
      ctx.globalAlpha = 1;
      ctx.fillRect(
        canvas.width / 2 - playa.x,
        canvas.height / 2 - playa.y,
        3000,
        1400
      );
      ...
      let oreEntities = [
        ...CTrees.findAll(
          (e) => Math.abs(e.y - playa.y) < 500 && Math.abs(e.x - playa.x) < 800
        ),
        ...Stones.findAll(
          (e) => Math.abs(e.y - playa.y) < 500 && Math.abs(e.x - playa.x) < 800
        ),
      ...
      dropped.forEach((item) => {
        if (
          item.slot.image == "stone" ||
          item.slot.image == "iron" ||
          item.slot.image == "gold" ||
          item.slot.image == "diamond" ||
          item.slot.image == "emerald" ||
          item.slot.image == "amethyst"
        ) {
          ctx.drawImage(
            Img[item.slot.image],
            item.x + x - 20,
        ...
        if (/^Leather$/.test(item.slot.type))
          ctx.drawImage(
            Img[item.slot.image],
            item.x + x - 24.5,
            item.y + y - 28.5,
        ...
        if (
          /Wall|Campfire|Door|Floor|Crafting Table|Chest|Armor/.test(
            item.slot.type
          )
        ) {
          ctx.save();
          ...
          if (item.slot.type == "Chest")
            ctx.drawImage(Img[item.slot.image], 0 - 23.75, 0 - 12.5, 47.5, 25);
      ...
      for (var i = 0; i < l; i++) {
        ...
        if (player.score > 1000000)
          player._score = Math.round(player.score / 100000) / 10 + "m";
        else if (player.score > 1000)
          player._score = Math.round(player.score / 100) / 10 + "k";
        else player._score = player.score;
        ctx.textStyle = "start";
        ctx.font = "15px Arial";
        ctx.fillStyle = "#521c18";
      ...
      if (playa.clanning) {
        ...
        if (playa.clan) {
          ...
          playa.clanMembers.forEach((player, i) => {
            let offSetX =
              ((i - Math.floor(i / 2) * 2) * (canvas.width - 1000)) / 2;
            let offSetY = Math.floor(i / 2) * 50;
            ctx.fillStyle = "white";
          ...
          playa.clans.forEach((name, i) => {
            let offSetX =
              ((i - Math.floor(i / 2) * 2) * (canvas.width - 820)) / 2;
            let offSetY = Math.floor(i / 2) * 30;
            ctx.textAlign = "start";
            ctx.font = "12px Arial";
            ctx.fillText(name, 410 + offSetX, 110 + 12 + 26 + offSetY);
      ...
      if (playa.crafting && playa.craftablesEx) {
        ...
        craftablesEx.forEach((craft, i) => {
          ...
          if (
            ...
            if (craft.craft == "Chest")
              ctx.drawImage(Img[img], 0 - 12.75, 0 - 7.5, 25.5, 15);
        ...
        playa.craftables.forEach((craft, i) => {
          ...
          if (
            /Wall|Campfire|Door|Floor|Crafting Table|Chest|Armor/.test(craft)
          ) {
            let img = craft.toLowerCase().replace(/\s/, "");
            ctx.globalAlpha = 0.875;
            ctx.lineWidth = 2;
      ...
      if (playa.chesting && playa.chestables) {
        ...
        playa.chestables.forEach((item, i) => {
          ...
          ctx.globalAlpha = 0.875;
          ctx.rect(
            (canvas.width - 300) / 2 + 40 + offSetX,
            (canvas.height - 300) / 2 + 40 + offSetY,
            60,
            60
          );
          ctx.stroke();
          ...
          if (
            /^(stone|iron|gold|diamond|emerald|amethyst|sand)$/.test(item.type)
          ) {
            ctx.save();
            ctx.translate(
              (canvas.width - 300) / 2 + offSetX + 70,
              (canvas.height - 300) / 2 + offSetY + 70
            );
            ctx.save();
            ctx.rotate(0);
            ctx.drawImage(
          ...
          if (/Axe|Pickaxe|Shovel|Sword|Hammer/.test(item.type)) {
            ...
            ctx.save();
            ctx.translate(
              (canvas.width - 300) / 2 + 40 + offSetX + 27.5,
              (canvas.height - 300) / 2 + 40 + offSetY + 27.5 + 5
            );
            ctx.rotate((Math.PI / 180) * 45);
          ...
          if (
            ...
            ctx.save();
            ctx.translate(
              (canvas.width - 300) / 2 + 40 + offSetX + 30,
              (canvas.height - 300) / 2 + 40 + offSetY + 30
            );
            ctx.rotate((Math.PI / 180) * 8);
            ...
            ctx.strokeText(
              item.count,
              (canvas.width - 300) / 2 + 40 + offSetX + 27.5 + 10,
              (canvas.height - 300) / 2 + 40 + offSetY + 60 - 6
            );
            ctx.fillText(
              item.count,
              (canvas.width - 300) / 2 + 40 + offSetX + 27.5 + 10,
      ...
      playa.inventory.forEach((slot, i) => {
        ...
        ctx.globalAlpha = 0.75;
        ctx.rect(
          canvas.width / 10 + (canvas.width / 10) * i - 45,
          canvas.height - 100 - 45,
          90,
          90
        );
        ctx.fill();
        ...
        ctx.strokeText(
          i + 1,
          canvas.width / 10 + (canvas.width / 10) * i - 45 + 3,
          canvas.height - 100 - 45 + 20
        );
        ctx.fillText(
          i + 1,
          canvas.width / 10 + (canvas.width / 10) * i - 45 + 3,
        ...
        if (/Axe|Pickaxe|Shovel|Sword|Hammer/.test(slot.type)) {
          ...
          ctx.save();
          ctx.translate(
            canvas.width / 10 + (canvas.width / 10) * i,
            canvas.height - 100 + 7
          );
          ctx.rotate((Math.PI / 180) * 45);
        ...
        if (
          /^(stone|iron|gold|sand|diamond|emerald|amethyst)$/.test(slot.image)
        ) {
          ctx.save();
          ctx.translate(
            canvas.width / 10 + (canvas.width / 10) * i,
          ...
          ctx.strokeText(
            slot.count,
            canvas.width / 10 + (canvas.width / 10) * i + 18,
            canvas.height - 58
          );
          ctx.fillText(
            slot.count,
        ...
        if (
          /Wall|Campfire|Door|Floor|Crafting Table|Chest|Armor/.test(slot.type)
        ) {
          ctx.save();
          ctx.translate(
            canvas.width / 10 + (canvas.width / 10) * i,
          ...
          ctx.rotate((Math.PI / 180) * 10);
          if (slot.type == "Chest")
            ctx.drawImage(Img[slot.image], 0 - 23.75, 0 - 12.5, 47.5, 25);
        ...
        if (slot.image == "draw" && slot.type == "wood") {
          ...
          ctx.strokeStyle = "maroon";
          ctx.moveTo(
            canvas.width / 10 + (canvas.width / 10) * i - 45 + 75,
            canvas.height - 100 - 45 + 27.857
          );
          ctx.lineTo(
            canvas.width / 10 + (canvas.width / 10) * i - 45 + 15,
            canvas.height - 100 - 45 + 62.143
          );
          ctx.stroke();
          ctx.lineJoin = ctx.lineCap = "round";
          ctx.lineWidth = 7;
      ...
      if (hover && hover[0] && playa.inventory[hover[1]].type) {
        if (hover[0].type == "Resource") {
          ...
          let w =
            ctx.measureText(desc).width > ctx.measureText(name).width
          ...
          ctx.strokeText(
            capitalize(name),
            5 + canvas.width / 10 + (canvas.width / 10) * hover[1] - 45,
            canvas.height - 100 - 45 - 55 + 20
          );
          ctx.strokeStyle = "grey";
          ctx.strokeText(
            desc,
            5 + canvas.width / 10 + (canvas.width / 10) * hover[1] - 45,
            canvas.height - 100 - 45 - 55 + 37
          );
      ...
      if (pack.tod == "day") {
        xclocke =
          Math.cos(((pack.per * 180 + 180) * Math.PI) / 180) * 45 +
          canvas.width -
          100;
        yclocke =
          Math.sin(((pack.per * 180 + 180) * Math.PI) / 180) * 45 +
          canvas.height -
          200;
        xclockr =
          Math.cos(((pack.per * 180 + 180 + 5) * Math.PI) / 180) * 35 +
          canvas.width -
          100;
        yclockr =
          Math.sin(((pack.per * 180 + 180 + 5) * Math.PI) / 180) * 35 +
          canvas.height -
          200;
        xclockl =
          Math.cos(((pack.per * 180 + 180 - 5) * Math.PI) / 180) * 35 +
        ...
        xclocke =
          Math.cos((pack.per * 180 * Math.PI) / 180) * 45 + canvas.width - 100;
        yclocke =
          Math.sin((pack.per * 180 * Math.PI) / 180) * 45 + canvas.height - 200;
        xclockr =
          Math.cos(((pack.per * 180 + 5) * Math.PI) / 180) * 35 +
          canvas.width -
          100;
        yclockr =
          Math.sin(((pack.per * 180 + 5) * Math.PI) / 180) * 35 +
          canvas.height -
          200;
        xclockl =
          Math.cos(((pack.per * 180 - 5) * Math.PI) / 180) * 35 +
