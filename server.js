...
if (process.env.NODE_ENV == "development") {
  httpsServer = https
    ...
    .listen(process.env.PORT, function () {
      console.log(
        "Express server listening with https on port %d in %s mode",
        this.address().port,
        app.settings.env
      );
    });
...
if (process.env.NODE_ENV == "development") {
  app.use(function (req, res, next) {
    ...
    if (!req.secure) {
      return res.redirect(
        301,
        "https://" + req.hostname + ":" + process.env.PORT + req.url
      );
    } else {
      return next();
    }
...
httpServer.listen(port, () => {
  console.log(
    "Your http server is listening on port " + httpServer.address().port
  );
});
    ...
    let str =
      ...
      `${prop}: ${
        typeof obj[prop] == "object"
          ? JSONtoString(obj[prop], indentTimes + 1)
          : obj[prop]
      }\r\n`;
    return str;
  };
...
if (process.env.NODE_ENV == "production") {
  let errorHandler = (e) => {
    let errorList = mongoDB.databases
      .get("collapsa")
      .collections.get("errorList");
